(() => {
  ...
  const Datastore = function () {
    ...
    this.source = () => ({ name: that.name, url: that.url, db: that.db });
    this.get = (key_or_query = {}) =>
      new Promise((resolve, reject) => {
        key_or_query = ccm.helper.clone(key_or_query);
        if (
          !ccm.helper.isObject(key_or_query) &&
          !ccm.helper.isKey(key_or_query)
        )
          return reject(new Error("invalid dataset key: " + key_or_query));
        that.url ? serverDB() : that.name ? clientDB() : localCache();
        function localCache() {
          resolve(
            ccm.helper.clone(
              ccm.helper.isObject(key_or_query)
                ? runQuery(key_or_query)
                : that.local[key_or_query],
            ),
          );
          function runQuery(query) {
            const results = [];
            for (const key in that.local)
              ccm.helper.isSubset(query, that.local[key]) &&
                results.push(that.local[key]);
            return results;
          }
        }
        function clientDB() {
          const store = getStore();
          const request = ccm.helper.isObject(key_or_query)
            ? store.getAll()
        ...
        function serverDB() {
          (that.socket ? useWebsocket : useHttp)(
            prepareParams({ get: key_or_query }),
          )
            .then(resolve)
    ...
    this.set = (priodata) =>
      new Promise((resolve, reject) => {
        ...
        if (
          !ccm.helper.isKey(priodata.key) &&
          !ccm.helper.isObject(priodata.key)
        )
          return reject(new Error("invalid dataset key: " + priodata.key));
        that.url ? serverDB() : that.name ? clientDB() : localCache();
        async function localCache() {
          if (that.local[priodata.key])
            that.local[priodata.key] = await ccm.helper.integrate(
              priodata,
    ...
    this.del = (key) =>
      new Promise((resolve, reject) => {
        if (!ccm.helper.isKey(key))
          return reject(new Error("invalid dataset key: " + key));
        that.url ? serverDB() : that.name ? clientDB() : localCache();
        function localCache() {
        ...
        function serverDB() {
          (that.socket ? useWebsocket : useHttp)(prepareParams({ del: key }))
            .then(resolve)
            .catch((error) => checkError(error, reject));
        }
      });
    this.count = (query) =>
      new Promise((resolve, reject) => {
        ...
        function localCache() {
          resolve(
            query
              ? that.get(query).then((datasets) => datasets.length)
              : Object.keys(that.local).length,
          );
        }
        function clientDB() {
          const request = getStore().count();
          request.onsuccess = (event) =>
            (!isNaN(event.target.result) ? resolve : reject)(
              event.target.result,
            );
          request.onerror = (event) => reject(event.target.errorCode);
        }
        function serverDB() {
          (that.socket ? useWebsocket : useHttp)(
            prepareParams(query ? { count: query } : {}),
          )
            .then((response) =>
              !isNaN(response) ? resolve(parseInt(response)) : reject(response),
            )
            .catch((error) => checkError(error, reject));
        }
      });
    ...
    function prepareRealtime() {
      return new Promise((resolve) => {
        ...
        if (that.dataset) {
          that.observe = that.dataset;
          delete that.dataset;
        }
        if (that.observe) {
          if (!Array.isArray(that.observe)) that.observe = [that.observe];
          that.observe = that.observe.map((key_or_query) =>
            ccm.helper.isObject(key_or_query)
              ? JSON.stringify(key_or_query)
              : key_or_query,
          );
        ...
        that.socket.onmessage = (message) => {
          const { callback: callback, data: data } = ccm.helper.parse(
            message.data,
          );
          if (callback) {
      ...
      return new Promise((resolve, reject) => {
        ...
        callbacks[key] = (result) =>
          Number.isInteger(result)
            ? checkError(result, reject)
            : resolve(result);
        params.callback = key;
        try {
          if (that.socket.readyState > 1)
            prepareRealtime().then(() =>
              that.socket.send(ccm.helper.stringify(params)),
            );
          else that.socket.send(ccm.helper.stringify(params));
        } catch (e) {
          prepareRealtime().then(() =>
            that.socket.send(ccm.helper.stringify(params)),
          );
  ...
  const ccm = {
    ...
    load: function () {
      ...
      const call = args.slice(0);
      call.unshift(ccm.load);
      let results = [];
      let counter = 1;
      let failed = false;
      return new Promise((resolve, reject) => {
        args.forEach((resource, i) => {
          ...
          if (!resource.context || resource.context === "head")
            resource.context = document.head;
          ...
          ccm.timeout &&
            window.setTimeout(
              () =>
                timeout === undefined && (timeout = true) && error("timeout"),
              ccm.timeout,
            );
          operation();
          ...
          function loadJSON() {
            ...
            function jsonp() {
              ...
              element.src = element.src.replace(/&amp;/g, "&");
              resource.context.appendChild(element);
            }
            function ajax() {
              ...
              request.open(
                resource.method,
                resource.method === "get" && resource.params
                  ? buildURL(resource.url, resource.params)
                  : resource.url,
                true,
              );
              if (resource.headers)
              ...
              request.onreadystatechange = () => {
                if (request.readyState === 4)
                  request.status >= 200 && request.status < 300
                    ? successData(request.responseText)
                    : error(request);
              };
              request.send(
                resource.method === "post" || resource.method === "put"
                  ? ccm.helper.stringify(resource.params)
                  : undefined,
              );
            }
            function fetchAPI() {
              ...
              if (resource.params)
                resource.init.method.toLowerCase() === "post"
                  ? (resource.init.body = ccm.helper.stringify(resource.params))
          ...
          function error() {
            failed = true;
            results[i] = {
              error: new Error("loading of " + resource.url + " failed"),
              resource: resource,
              data: [...arguments],
              call: call,
            };
            if (results[i].data.length <= 1)
    ...
    component: async (component, config) => {
      ...
      const version = (
        ccm.helper.isCore(component.ccm)
          ? component.ccm
          : await ccm.helper.loadVersion(component.ccm)
      ).version();
      if (version !== ccm.version())
        return new Promise(async (resolve) => {
          const result = await window.ccm[version].component(
            component,
            config,
            resolve,
          );
      ...
      if (!_components[component.index]) {
        ...
        component.ccm = window.ccm[version];
        component.ready && (await component.ready.call(component));
      ...
      component.instance = async (config) =>
        await ccm.instance(
          component,
          await prepareConfig(config, component.config),
        );
      component.start = async (config) =>
        await ccm.start(
          component,
          await prepareConfig(config, component.config),
        );
      ...
      async function getComponentObject() {
        if (typeof component === "string") {
          ...
          if (ccm.helper.regex("filename").test(component.split("/").pop())) {
            const response = await ccm.load(component);
            response.url = component;
            return response;
          }
    ...
    instance: async (component, config) => {
      if (config && config.root && config.parent) {
        config.root.innerHTML = "";
        config.root.appendChild(ccm.helper.loading(config.parent));
      }
      component = await ccm.component(component, { ccm: config && config.ccm });
      config && delete config.ccm;
      config = await prepareConfig(config, component.config);
      config.beforeCreation &&
        (await config.beforeCreation(config, ccm.helper.clone(component)));
      delete config.beforeCreation;
      if (!ccm.helper.isComponent(component)) return component;
      if (component.ccm.version() !== ccm.version())
        return new Promise(async (resolve) => {
          const result = await window.ccm[component.ccm.version()].instance(
            component,
            config,
            resolve,
          );
      ...
      config.afterCreation &&
        (await config.afterCreation(config, ccm.helper.clone(component)));
      delete config.afterCreation;
      ...
      if (!document.contains(instance.root)) {
        ...
        if (instance.root.parentNode)
          instance.root.parentNode.replaceChild(
            instance.root.position,
            instance.root,
          );
      ...
      if (document.head.contains(instance.root)) {
        ...
        if (instance.root.position.parentNode)
          instance.root.position.parentNode.replaceChild(
            instance.root,
            instance.root.position,
          );
      ...
      instance.onReady && (await instance.onReady(instance));
      delete instance.onReady;
      return instance;
      function createInstance() {
        const instance = new component.Instance();
        instance.ccm = component.ccm;
        instance.component = component;
        instance.parent = config.parent;
        delete config.parent;
        instance.children = {};
        instance.root = config.root;
        delete config.root;
        instance.id = ++_components[component.index].instances;
        instance.index = component.index + "-" + instance.id;
        setElement();
        ...
        if (
          ...
          instance.root.parentNode.tagName.indexOf("CCM-") === 0
        )
          watchAttributes();
        return instance;
        function setElement() {
          ...
          if (typeof instance.root === "string")
            instance.root = instance.parent.element.querySelector(
              "#" +
                (instance.root === "name" && instance.parent
                  ? component.name
                  : instance.root),
            );
          ...
          if (instance.root) {
            ...
            instance.root.appendChild(root);
            instance.root = root;
          }
          if (!config.shadow) config.shadow = "closed";
          if (typeof config.shadow === "string" && config.shadow !== "none") {
            instance.shadow =
              root.shadowRoot || root.attachShadow({ mode: config.shadow });
            delete config.shadow;
          }
          (instance.shadow || root).appendChild(
            (instance.element = ccm.helper.html({ id: "element" })),
          );
          config.breakpoints !== false &&
            ccm.helper.responsive(
              instance.element,
              config.breakpoints,
              instance,
            );
          ...
          if (instance.start) {
            ...
            instance.element.appendChild(ccm.helper.loading(instance));
          }
        }
            ...
            instance.update = (key, value) => {
              switch (key) {
                ...
                case "update":
                  break;
                case "key":
                  if (ccm.helper.regex("json").test(value))
                    value = ccm.helper.parse(value);
    ...
    proxy: async (component, config) => {
      ...
      obj.start = async (cfg) =>
        await Object.assign(
          ...
          await ccm.instance(
            component,
            await ccm.helper.integrate(cfg, config),
          ),
        ).start();
      return obj;
    },
    start: async (component, config = {}, element) => {
      if (element) config.root = element;
    ...
    store: (config = {}) =>
      new Promise((resolve, reject) => {
        ...
        if (typeof config === "string")
          config = config.split("?").shift().endsWith(".js")
            ? { local: ["ccm.load", config] }
            : { name: config };
        if (!ccm.helper.isObject(config) || (!config.local && !config.name)) {
          config = { local: config, parent: config.parent };
          delete config.local.parent;
        }
        if (!config.local && !config.name) config.local = {};
        ccm.helper
          .solveDependency(config.local)
          .then((result) => {
            config.local = result;
            if (Array.isArray(config.local))
              config.local = ccm.helper.arrToStore(config.local);
    ...
    get: (config = {}, key_or_query = {}) =>
      ccm.store(config).then((store) => {
        let property;
        ...
        return store
          .get(key_or_query)
          .then((result) =>
            property ? ccm.helper.deepValue(result, property) : result,
          );
      }),
    components: {},
    context: {
    ...
    helper: {
      ...
      clone: function (value) {
        ...
        function recursive(value, first) {
          ...
          if (Array.isArray(value) || ccm.helper.isObject(value)) {
            ...
            for (var i in value) copy[i] = recursive(value[i]);
            return copy;
          }
          return value;
        }
      ...
      convertComponentURL: (url) => {
        ...
        const data = { url: url };
        let tmp = filename.substring(4, filename.length - 3);
        if (tmp.endsWith(".min")) {
          data.minified = true;
        ...
        tmp = tmp.split("-");
        data.name = tmp.shift();
        if (tmp.length) data.version = tmp[0];
        data.index =
          data.name +
      ...
      format: function (data, values) {
        ...
        values = ccm.helper.clone([...arguments]);
        values.shift();
      ...
      generateConfig: (element) => {
        ...
        if (ccm.helper.regex("json").test(element.innerHTML)) {
        ...
        function catchAttributes(element, obj) {
          ...
          [...element.attributes].forEach((attr) => {
            if (
              ...
              try {
                obj[attr.name] =
                  attr.value.charAt(0) === "{" || attr.value.charAt(0) === "["
                    ? ccm.helper.parse(attr.value)
                    : prepareValue(attr.value);
              } catch (err) {}
          });
        }
        function catchInnerTags(element) {
          ...
          [...element.childNodes].forEach((child) => {
            if (child.tagName && child.tagName.indexOf("CCM-") === 0) {
              ...
              switch (split[1]) {
                case "load":
                  ccm.helper.deepValue(
                    config,
                    split[2],
                    interpretLoadTag(child, split[2]),
                  );
                  break;
                case "component":
                case "instance":
                case "proxy":
                  ccm.helper.deepValue(config, split[2], [
                    "ccm." + split[1],
                    child.getAttribute("src") || split[2],
                    ccm.helper.generateConfig(child),
                  ]);
                  break;
                case "store":
                case "get":
                ...
                case "list":
                  ...
                  [...child.children].forEach((entry) => {
                    if (
                      entry.tagName &&
                      entry.tagName.indexOf("CCM-ENTRY") === 0
                    ) {
                      const value = prepareValue(entry.getAttribute("value"));
                      const split = entry.tagName.toLowerCase().split("-");
                      if (!list) list = split.length < 3 ? [] : {};
                      if (split.length < 3) list.push(value);
                      else ccm.helper.deepValue(list, split[2], value);
                    }
                  });
                  if (!list) list = {};
          ...
          function interpretLoadTag(node) {
            ...
            function generateParameters(node) {
              if (node.hasAttribute("src")) {
                if (node.children.length === 0) return node.getAttribute("src");
                const data = {};
                [...node.children].forEach((child) => {
                  if (child.tagName && child.tagName.indexOf("CCM-DATA-") === 0)
                    data[child.tagName.toLowerCase().split("-")[2]] =
              ...
              [...node.children].forEach((child) => {
                if (
                  (child.tagName === "CCM-SERIAL" &&
                    (node.tagName === "CCM-PARALLEL" ||
                      node.tagName.indexOf("CCM-LOAD") === 0)) ||
                  (child.tagName === "CCM-PARALLEL" &&
                    node.tagName === "CCM-SERIAL")
                )
      ...
      html: function (html, ...values) {
        ...
        if (
          values.length > 1 &&
          ccm.helper.isObject(values[values.length - 1])
        ) {
          advanced = values.pop();
        ...
        if (Array.isArray(html)) {
          ...
          for (let i = 0; i < html.length; i++)
            result.push(ccm.helper.html(html[i], undefined, advanced));
          return result;
        }
        if (typeof html !== "object" || html === null)
          html = { tag: "span", inner: html };
        if (html.tag === "svg")
          advanced.namespace_uri = "http://www.w3.org/2000/svg";
        const element = advanced.namespace_uri
          ? document.createElementNS(advanced.namespace_uri, html.tag || "div")
          : document.createElement(html.tag || "div");
        delete html.tag;
        if (!ccm.helper.regex("json").test(html.key)) delete html.key;
          ...
          switch (key) {
            ...
            case "inner":
              ...
              children.forEach((child) =>
                element.appendChild(this.html(child, undefined, advanced)),
              );
              break;
            default:
        ...
        if (element.tagName.startsWith("CCM-") && !advanced.no_evaluation) {
          ...
          config.root = element;
          (config.ccm && config.ccm !== "latest"
            ? window.ccm[config.ccm]
            : window.ccm
          ).start(
            element.tagName === "CCM-APP"
              ? element.getAttribute("component")
              : element.tagName.substr(4).toLowerCase(),
            config,
          );
        }
        return element;
      ...
      integrate: async (priodata, dataset, as_defaults) => {
        ...
        for (let key in priodata) {
          ...
          if (!as_defaults || value === undefined || value === "")
      ...
      isCore: (value) =>
        ccm.helper.isObject(value) && value.components && value.version && true,
      isInstance: (value) =>
        ccm.helper.isObject(value) &&
        ccm.helper.isComponent(value.component) &&
        value.ccm &&
        true,
      isKey: (value) => {
        if (typeof value === "string" || typeof value === "number")
          return ccm.helper.regex("key").test(value);
        if (Array.isArray(value)) {
          for (let i = 0; i < value.length; i++)
            if (!ccm.helper.regex("key").test(value[i])) return false;
          return true;
        }
        return false;
      },
      ...
      isSpecialObject: (value) =>
        !!(
          ...
          (window.jQuery && value instanceof jQuery)
        ),
      isSubset(obj, other) {
        for (const key in obj)
          if (obj[key] === null) {
      ...
      loadVersion: async (url) => {
        ...
        const version = (resource.url.match(
      ...
      responsive: (
        ...
        function updateBreakpoints() {
          ...
          for (const key in breakpoints)
            element.classList[
              element.offsetWidth >= breakpoints[key] ? "add" : "remove"
            ](key);
          const after = element.getAttribute("class");
          !init &&
            before !== after &&
            instance.onbreakpoint &&
            instance.onbreakpoint(breakpoints[after.split(" ").pop()] || 0);
          init = false;
        }
      },
      solveDependencies: (obj, instance) =>
        new Promise((resolve, reject) => {
          ...
          if (!ccm.helper.isObject(obj) && !Array.isArray(obj))
            return resolve(obj);
          let failed = false;
          let counter = 1;
          search(obj);
      ...
      solveDependency: async (dependency, instance) => {
        ...
        switch (operation) {
          ...
          case "set":
          case "del":
            dependency[1] = await ccm.helper.solveDependency(
              dependency[1],
              instance,
            );
            if (!dependency[operation === "store" ? 0 : 1])
              dependency[operation === "store" ? 0 : 1] = {};
            if (instance)
              dependency[operation === "store" ? 0 : 1].parent = instance;
      ...
      toDotNotation: function (obj, all_levels) {
        ...
        function recursive(obj, prefix) {
          for (const key in obj) {
            if (
              typeof obj[key] === "object" &&
              !ccm.helper.isSpecialObject(obj[key])
            ) {
  ...
  async function defineCustomElement(name) {
    if (!("customElements" in window))
      await ccm.load({
        ...
        attr: {
          integrity:
            "sha384-yEuTKRGFLhOAfHNxaZiiI23KhMelYudrPUNSUK6T5u1+deGEEKsQ89cS0sPIHjyj",
      ...
      class extends HTMLElement {
        async connectedCallback() {
          ...
          while ((node = node.parentNode))
            if (node.tagName && node.tagName.indexOf("CCM-") === 0) return;
          const config = ccm.helper.generateConfig(this);
          config.root = this;
          await (config.ccm && config.ccm !== "latest"
            ? window.ccm[config.ccm]
            : window.ccm
          ).start(this.tagName === "CCM-APP" ? config.component : name, config);
        }
      },
    );
  }
  ...
  async function changeVersion(component, config) {
    const source =
      (config && config.ccm) || (component.config && component.config.ccm);
    if (source) {
      component.ccm =
        source === "latest"
          ? window.ccm
          : window.ccm[source] ||
            window.ccm[(await ccm.helper.loadVersion(source)).version()];
      component.ccm.url = ccm.helper.isObject(source) ? source.url : source;
      config && delete config.ccm;
      component.config && delete component.config.ccm;
    }
  }
})();
//# sourceMappingURL=ccm-27.5.0.min.js.map
